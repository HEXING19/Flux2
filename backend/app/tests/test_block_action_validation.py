...
from app.skills.entity_skill import EntityQuerySkill
...
class BlockSkillValidationTest(unittest.TestCase):
    ...
    def setUp(self):
        self.ctx = SkillContextManager(enable_persistence=False)
        self.online_devices = [
    ...
    def test_block_action_should_reject_invalid_ip_view(self):
        skill = BlockActionSkill(FakeRequester(online_devices=self.online_devices), self.ctx)
        with self.assertRaises(ValidationGuardException):
            skill.execute(
                "s1",
    ...
    def test_entity_query_should_reject_invalid_ip(self):
        skill = EntityQuerySkill(FakeRequester(), self.ctx)
        with self.assertRaises(ValidationGuardException):
            skill.execute("s1", {"ips": ["invalid-ip"]}, "查询 invalid-ip 情报")

    def test_block_query_empty_returns_unblocked_with_quick_action(self):
        ...
        self.assertEqual(payloads[1]["type"], "quick_actions")
        self.assertEqual(payloads[1]["data"]["actions"][0]["message"], "封禁 200.200.1.1")
    ...
    def test_block_action_should_reject_protected_target_before_form(self):
        with Session(engine) as session:
            session.add(SafetyGateRule(rule_type="ip", target="20.1.1.1", description="test"))
            session.commit()

        skill = BlockActionSkill(FakeRequester(online_devices=self.online_devices), self.ctx)
        with self.assertRaises(ValidationGuardException):
            skill.execute("s1", {"views": ["20.1.1.1"]}, "封禁20.1.1.1这个IP地址")
